  ...
  return (
    <header className="fixed top-0 right-0 left-0 md:left-64 h-16 bg-surface-dim/80 backdrop-blur-md z-30 flex items-center justify-between px-grid-gutter">
      <h1 className="md:hidden">
        <Link
          href="/"
          className="text-headline-md font-bold text-primary rounded focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
        >
          gameColector
        </Link>
      </h1>
      <div className="ml-auto flex items-center gap-stack-sm">
        {user ? (
          <>
            <span className="text-label-md text-on-surface-variant hidden sm:inline md:hidden">
              {user.name}
            </span>
            <SignOutForm
              className="md:hidden"
